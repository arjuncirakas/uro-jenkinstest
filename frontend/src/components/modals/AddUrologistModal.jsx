...
import { X, Users, Mail, Phone, Building2, User } from 'lucide-react';
...
const AddUrologistModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const [urologyDepartmentId, setUrologyDepartmentId] = useState(null);
  ...
  const fetchDepartments = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Find urology department
        const urologyDept = response.data.find(dept => 
          dept.name && dept.name.toLowerCase().trim() === 'urology'
        );
        if (urologyDept) {
          setUrologyDepartmentId(urologyDept.id);
          setFormData(prev => ({ ...prev, department_id: urologyDept.id.toString() }));
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setErrorMessage(response.error || response.message || 'Failed to create urologist');
        setShowErrorModal(true);
      }
    } catch (error) {
      console.error('Error creating urologist:', error);
      const message = error?.response?.data?.error || 
                     error?.response?.data?.message || 
                     error?.message || 
                     'Failed to create urologist. Please try again.';
  ...
  return (
    <>
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-xl font-semibold text-gray-900">Add New Urologist</h2>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="flex space-x-3 pt-4">
              ...
              >
                {isLoading ? 'Adding...' : 'Add Urologist'}
              </button>
            </div>
          </form>
        </div>
      ...
      <SuccessModal
        ...
        title="Urologist Added Successfully"
        message="The urologist has been added to the doctors table. A password setup email has been sent."
      />
