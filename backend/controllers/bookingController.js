...
// Get today's appointments for dashboard (simplified version)
export const getDashboardTodaysAppointments = async (req, res) => {
  const requestId = `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  console.log(`\n🎯 [getDashboardTodaysAppointments ${requestId}] Starting`);
  
  let client;
  try {
    client = await pool.connect();
  } catch (dbError) {
    console.error(`❌ [getDashboardTodaysAppointments ${requestId}] Database connection failed:`, dbError.message);
    return res.status(503).json({
      success: false,
      message: 'Database connection failed',
      error: 'Service temporarily unavailable'
  ...
  try {
    ...
    // For urologists/doctors, get their doctors.id
    let doctorId = null;
    if (userRole === 'urologist' || userRole === 'doctor') {
      const doctorCheck = await client.query(
        'SELECT id FROM doctors WHERE email = $1 AND is_active = true',
    ...
    if ((userRole === 'urologist' || userRole === 'doctor') && doctorId) {
      ...
      query = `
        SELECT 
          ...
          a.status,
          a.notes,
          COALESCE(a.appointment_type, 'urologist') as type
        FROM appointments a
        JOIN patients p ON a.patient_id = p.id
        WHERE a.appointment_date = $1
        AND a.urologist_id = $2
    ...
    console.log(`✅ [getDashboardTodaysAppointments ${requestId}] Returning ${formattedAppointments.length} appointments`);
    ...
    console.error(`❌ [getDashboardTodaysAppointments ${requestId}] Error:`, error.message);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
