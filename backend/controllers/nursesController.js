...
export const createNurse = async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      ...
      [email, passwordHash, first_name, last_name, phone, organization || null, ROLE, false, false]
    );
...
export const updateNurse = async (req, res) => {
  ...
  try {
    ...
    if (await checkPhoneExists(client, phone, email || oldEmail)) {
      await client.query('ROLLBACK');
      return createErrorResponse(res, 409, 'Phone number is already in use');
    }
    ...
    `, [first_name, last_name, email, phone, organization || null, finalIsActive, id, ROLE]);
